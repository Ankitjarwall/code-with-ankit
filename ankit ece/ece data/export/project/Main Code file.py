    ...
    try:
        ...
        if (positionNumber == -1):
            ...
            if (calendar.day_name[today_name.weekday()] != 'Sunday'):
                ...
                ## Create a Hash Value for the finger
                ##
                ...
                if (now <= time_9) and (now > time_7):
                    con = sqlite3.connect('/home/pi/Desktop/Biometric-Attendance-System-Mod/attendance/app.db')
                    c = con.cursor()
                    c.execute('UPDATE attendance SET statusfirst = ? WHERE (rollnum, date) in (values(?, ?))',
                    ...
                    mylcd.lcd_display_string("You are late", 2, 2)
                    sleep(2)
                    mylcd.lcd_clear()
                    ...
                    con = sqlite3.connect('/home/pi/Desktop/Biometric-Attendance-System-Mod/attendance/app.db')
                    c = con.cursor()
                    c.execute('UPDATE attendance SET statuslab = ? WHERE (rollnum, date) in (values(?, ?))',
                              ('present', ext_id, today_name))
                    con.commit()
                    con.close()
